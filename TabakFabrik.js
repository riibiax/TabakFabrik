var TabakFabrik = (function() {
	...
	/*function createSpaceDistorsion() {
		var scene = new THREE.Scene();
		scene.name=3;
		var spaceUniforms = {
				delta:		{ type: "f", value: 0.1 },
		...
		var spaceGeometry =  new THREE.BoxGeometry( 150, 150, 600, 32, 32, 128 );
		...
		spacePoints = new THREE.Points( spacePointsGeometry, spacePointsMaterial );
		space = new THREE.Mesh(spaceGeometry, spaceMaterial);
		spacePoints.position.z = 3675;
		space.position.z = 3675;
		scene.add(spacePoints);
		scene.add(space);
		//SCENE 3
		scenes.push(scene);
	...
	/*function resetBoxTransition() {
    	var box = document.getElementById('fade').style;
		box.width ="100%";
		box.height ="100%";
		box.opacity=0.0;
	}
	...
	function setFlickering(color, freq, times) {
		...
		box.height ="100%";
		box.opacity=0.0;
		var fadeTween = new TWEEN.Tween( box )
            .to( { opacity: 1.0 }, freq )
            .repeat( times+1 )
            .yoyo( true )
            .easing( TWEEN.Easing.Linear.None )
            .onComplete(function () {resetBoxTransition();})
    		.start();
	}

	function setFadeInTransition(color, timing, direction) {
		var fadeTween;
		var box = document.getElementById('fade').style;
		box.backgroundColor= color;
		box.opacity= 1.0;
		...
		switch (direction) {
			case "right":
				fadeTween = new TWEEN.Tween( {value:100} )
            	.to( { value:0}, timing )
            	.onUpdate(function () {
	...
	function setFadeOutTransition(color, timing, direction) {
		var fadeTween;
		var box = document.getElementById('fade').style;
		box.backgroundColor= color;
		box.opacity= 1.0;
		switch (direction) {
			case "left":
			    box.width ="0%";
			    box.height ="100%";
				fadeTween = new TWEEN.Tween( {value:0} )
            	.to( { value:100}, timing )
            	.onUpdate(function () {
						box.width =this.value+"%";
					})
